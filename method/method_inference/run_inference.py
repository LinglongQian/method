...
def run_inference(loader, args, num_gpus: int = 8):
    ...
    toi = th.tensor(vocab.encode(stoi), device='cpu', dtype=th.long)
    model.eval()
    results = []
    for patient_timeline, time_indices, patient_indices, ground_truth in tqdm(
        loader, proc_name, total=len(loader), position=proc_num, smoothing=0
    ):
        patient_timeline = patient_timeline.to(device)
        time_indices = time_indices.to(device)
        patient_indices = patient_indices.to(device)
        gen_token_num = 0
        offset = 0
        if model.config.fix_sw_size:
            sw_size = 1024
        else:
            sw_size = int(round((1024 + (2048 - 1024) * min(len(patient_timeline) / model.config.sequence_length, 1.0)) / 128) * 128)
        while True:
            if test_name == Test.SOFA_PREDICTION and gen_token_num == 1:
                # append a sofa token to the timeline and continue generating
                last_token = th.tensor(vocab.encode(["SOFA"]), device=patient_timeline.device, dtype=th.long)
            else:
                outputs = model.get_next_token(
                                        inputs=patient_timeline, 
                ...
                last_token, probs = outputs["next_token"], outputs["probs"]
                if model.config.timeaware:
                    last_token_time = outputs["next_time"]
                else:
                    last_token_time = time_indices[-1]

            if not offset and len(patient_timeline) == timeline_len:
                offset = 1
            time_indices = th.cat(
                (time_indices[:context_len], time_indices[context_len + offset :], last_token_time.view(-1)),
            )
            patient_timeline = th.cat(
                (patient_timeline[:context_len], patient_timeline[context_len + offset :], last_token.view(-1)),
            )
            patient_indices = th.cat(
                (patient_indices[:context_len], patient_indices[context_len + offset :], patient_indices[-1].view(-1)),
            )
            gen_token_num += 1
            # stop criterion
            if patient_timeline[-1].cpu() in toi:
                ...
                timeline_time = vocab.get_timeline_total_time(
                    patient_timeline[-gen_token_num:].cpu(), decode=True
                )
        ...
        actual_prob = probs[last_token.item()].item()
        toi_probs = dict(zip(stoi, probs[toi].tolist()))
        timeline_time = vocab.get_timeline_total_time(patient_timeline[-gen_token_num:].cpu(), decode=True)
