...
from .mimiciv.special import MimicPatientBirthDateData
...
def load_and_process_data(
    ...
    module = import_module(f".{data_prop.module}.preprocessors", package="method.ethos_tokenize")
    registered_data = [
        cls
...
def get_context_data(data_prop, vocab, **kwargs):
    ...
    for i, data_cls in enumerate(context_data, 1):
        start_time = time.time()
        prev_vocab_size = len(vocab)
...
def dump_timelines(timelines_path, times, tokens, patient_idx_to_id, context_data, age_reference):
    with h5py.File(timelines_path, "w") as f:
        f.create_dataset("times", data=times, chunks=True, compression="gzip")
        f.create_dataset("tokens", data=tokens, chunks=True, compression="gzip")
        ...
        f.create_dataset("patient_context", data=patient_context, chunks=True, compression="gzip")
