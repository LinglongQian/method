...
#     def _process(self, df: pd.DataFrame) -> dict[int, np.ndarray]:
#         df = df[~df.result_name.isna() & df.result_name.str.startswith("BMI")].copy()
#         df = df.drop(columns="result_name")
#         with QStorageContext("BMI", self.vocab) as q_storage:
#             df.loc[:, "result_value"] = self._convert_to_deciles(
#                 q_storage, df.result_value.astype(float), "BMI"
#             )
#         return df
