...
from method.method_train import method_training
...
@option("--wandb_project", default="method")
@option("--wandb_run_name", default="method_run")
...
@option("--timeaware", is_flag=True, help="Use time-aware model.")
@option("--wta", is_flag=True, help="Time is completely irrelevant.")
@option("--check_model_precision", is_flag=True, help="Check model precision.")
@option("--weight_tying", is_flag=True, help="Use weight tying.")
@option("--fix_sw_size", is_flag=True, help="Fix the sliding window size.")
@option("--sw_size", type=int, default=1024, help="Sliding window size.")
...
@option("--data_type", type=Choice(["ethos", "method"]), default="method")
...
def train_method(**kwargs):
    ...
    $ torchrun --standalone --nproc_per_node=4 method train [args...]
    ...
    $ torchrun --nproc_per_node=8 --nnodes=2 --node_rank=0 --master_addr=123.456.123.456
     --master_port=1234 method train [args...]
