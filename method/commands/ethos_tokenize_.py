...
from method.constants import PROJECT_DATA
from method.ethos_tokenize import Vocabulary
from method.ethos_tokenize.constants import Dataset, DataFold, DataProp
from method.ethos_tokenize.tokenization_steps import (
...
from method.utils import convert_years, convert_seconds, get_logger

logger = get_logger()
DEFAULT_OUT_DIR = "ethos_tokenized_datasets"
...
def ethos_tokenize_(
    ...
    timelines_path = output_dir / f"{dataset_prop.dataset_dir}_{fold}_timelines_p{len(patient_idx_to_id)}.hdf5"
    ...
    if vocab_path is None:
        vocab_path = output_dir / f"{dataset_prop.dataset_dir}_vocab_t{len(vocab)}.pkl"
    else:
        vocab_path = output_dir / f"{dataset_prop.dataset_dir}_updated_vocab_t{len(vocab)}.pkl"
