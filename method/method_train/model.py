...
class TimeAwareRotary(nn.Module):

    def __init__(self, dim, max_seq_len=65536):
        super().__init__()
        # Half-truncate RoPE dimension for efficiency
        angular_freq = (1 / 1024) ** torch.linspace(0, 1, steps=dim//4, dtype=torch.float32)
        # Zero-pad the remaining frequencies
    ...
    def forward(self, x, time_indices):
        ...
        Args:
            ...
            time_indices: Time indices tensor
        Returns:
            Tensor of same shape as x with rotary embeddings applied
        """
        # Get the sin/cos values for the requested positions
        cos = self.cos[time_indices][None, :, None, :]  # [1, seq_len, 1, dim/2]
        sin = self.sin[time_indices][None, :, None, :]  
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, config):
        ...
        # Rotary embeddings
        self.rotary = TimeAwareRotary(dim // self.num_heads)
    ...
    def forward(self, x, vi, block_mask, time_indices):
        ...
        q, k = self.rotary(x=q, time_indices=time_indices), self.rotary(x=k, time_indices=time_indices)
...
class Block(nn.Module):
    ...
    def forward(self, x, vi, x0, block_mask, time_indices):
        # Weighted residual connection
        x = self.lambdas[0] * x + self.lambdas[1] * x0
        # Attention with RMS norm and block_mask
        x = x + self.attn(x=rms_norm(x), vi=vi, block_mask=block_mask, time_indices=time_indices)
        # MLP with RMS norm
...
class Method(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.timeaware:
            self.time_head = CastedLinear(config.model_dim, 1, config=config)
    ...
    def forward(self,
        ...
        def patient_timeaware_causal_mask(b, h, q_idx, kv_idx):
            temporal_causal_mask = time_indices[q_idx] >= time_indices[kv_idx]
            ...
            return (temporal_causal_mask | context_mask) & patient_mask & window_mask

        block_mask = create_block_mask(
            patient_timeaware_causal_mask, None, None, 
        ...
        for i in range(self.num_encoder_layers):
            x = self.transformer.h[i](x, ve_enc[i], x0, block_mask, time_indices)
            skip_connections.append(x)
        # Decoder pass with weighted skip connections
        for i in range(self.num_decoder_layers):
            x = x + self.skip_weights[i] * skip_connections.pop()
            x = self.transformer.h[self.num_encoder_layers + i](x, ve_dec[i], x0, block_mask, time_indices)
        ...
        if target is not None:
            ...
            if self.config.timeaware:
                time_labels = (target_time_indices - time_indices).float()
                time_loss = F.binary_cross_entropy_with_logits(
                    time_logits.view(-1),
                    time_labels.view(-1),
                    reduction='none',
                )
                time_loss = time_loss * valid_positions.float()
                time_loss = time_loss.sum() / num_valid
                loss = loss + time_loss
        else:
            loss = None
        ...
        if self.config.timeaware:
            outputs['time_logits'] = time_logits

        return outputs

    def _get_uncertain_mask(self, time_indices, patient_indices):
        """ 
        build a mask to mark uncertain predictions based on possible scenarios:
        1. patient boundary
        2. large time gap
        3. special events
        """
        uncertain_mask = torch.zeros_like(time_indices, dtype=torch.bool)
        
        # rule example: predictions across patient boundaries are marked as uncertain
        patient_boundaries = patient_indices[:, 1:] != patient_indices[:, :-1]
        uncertain_mask[:, :-1] |= patient_boundaries
        
        # rule example: predictions with large time gaps are marked as uncertain
        time_gaps = time_indices[:, 1:] - time_indices[:, :-1]
        large_gaps = time_gaps > self.config.time_threshold
        uncertain_mask[:, :-1] |= large_gaps
    ...
    def generate(
        ...
        for _ in range(max_new_tokens):
            ...
            if self.config.timeaware:
                ...
                has_time_step = torch.sigmoid(time_logits[0, real_pos]) > 0.5
                next_time = time_indices[-1] + has_time_step
                time_indices = torch.cat((time_indices, next_time), dim=0)
                return inputs, time_indices
    ...
    def get_next_token(
        ...
        if self.config.timeaware:
            time_logits = outputs['time_logits']
            has_time_step = torch.sigmoid(time_logits[0, -1, :]) > 0.5
            next_time = time_indices[-1] + has_time_step
            infer_outputs['next_time'] = next_time
