...
def load_model_from_checkpoint(path, device, wta, for_training=True, **kwargs):
    if wta:
        from .model_WTA import Method, ModelConfig
        print("Loading time-agnostic model")
    else:
        from .model import Method, ModelConfig
        print("Loading time-aware model")
    ...
    if not for_training:
        return model, checkpoint["config"].sequence_length
