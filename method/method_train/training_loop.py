...
from .metrics import Timeaware_estimate_loss
from .utils import get_train_val_data, make_infinite_loader, get_lr, load_model_from_checkpoint
from .datasets import TimeAwareDataset
...
torch._dynamo.config.verbose = True # display verbose output
torch._dynamo.config.suppress_errors = True # suppress errors
torch._inductor.config.debug = True # display debug output
torch._dynamo.config.replay_record_enabled = True


def method_training(args):
    ...
    if ddp:
        ...
        master_process = True
        seed_offset = 0
    tokens_per_iter = args.gradient_accumulation_steps * args.sequence_length
    ...
    source_file1 = './methods/method_train/training_loop.py'
    shutil.copy(source_file1, destination_folder)

    ctx = setup_torch(device, args.dtype, 42 + seed_offset)

    # VOCABULARY
    if args.data_type == "method":
        from methods.method_tokenize import SpecialToken, QITVocabulary
        vocab = QITVocabulary(args.vocab)
    else:
        from methods.ethos_tokenize import SpecialToken, Vocabulary
    ...
    train_dataset = TimeAwareDataset(train_data, encode=vocab.encode, sequence_length=args.sequence_length)
    val_dataset = TimeAwareDataset(val_data, encode=vocab.encode, sequence_length=args.sequence_length)
    ...
    if master_process:
        ...
        print(
            "Every evaluation will use {:.2%} of validation dataset".format(
                args.sequence_length * args.eval_iters / len(val_dataset)
            )
        )
    def get_batch(split: str = "train"):
        loader = train_dataloader if split == "train" else val_dataloader
        x, time_indices_x, patient_indices, y, time_indices_y = next(loader)
        return (
            x.squeeze().to(device, non_blocking=True),
            time_indices_x.squeeze().to(device, non_blocking=True),
            patient_indices.squeeze().to(device, non_blocking=True),
            y.squeeze().to(device, non_blocking=True),
            time_indices_y.squeeze().to(device, non_blocking=True),
        )
    ...
    model_args = dict(
        ...
        check_model_precision=args.check_model_precision,
        weight_tying=args.weight_tying,
        fix_sw_size=args.fix_sw_size,
    ...
    if args.resume:
        ...
        model, iter_num, best_val_loss, optimizer_state = load_model_from_checkpoint(
            model_path, device, args.wta
        )
    else:
        print("Initializing a new model from scratch")
        if args.wta:
            from .model_WTA import Method, ModelConfig
            source_file2 = './methods/method_train/model_WTA.py'
            shutil.copy(source_file2, destination_folder)
            print("Using time-agnostic model")
        else:
            from .model import Method, ModelConfig
            source_file2 = './methods/method_train/model.py'
            shutil.copy(source_file2, destination_folder)
            print("Using time-aware model")
    ...
    # optimizer
    optimizer = model.configure_optimizers_muon(
        args.weight_decay, args.lr, (args.beta1, args.beta2), device
    )
    if optimizer_state is not None:
        for opt, opt_state in zip(optimizer, optimizer_state):
    ...
    # training loop
    x, time_indices_x, patient_indices, y, time_indices_y = get_batch("train")  # fetch the very first batch
    t0 = time.time()
    local_iter_num = 0  # number of iterations in the lifetime of this process
    raw_model = model.module if ddp else model  # unwrap DDP container if needed
    running_mfu = -1.0
    sw_size = args.sw_size # initial sliding window size
    if args.fix_sw_size:
        print(f"Sliding window size fixed to {sw_size}")
    ...
    while True:
        ...
        # training progress
        if not args.fix_sw_size:
            frac_done = iter_num / args.max_iters
            step_sw_size = int(frac_done * (2048 - 1024) / 128) * 128 + 1024
            if step_sw_size != sw_size:
                sw_size = step_sw_size
                print(f"Step size changed to {sw_size}")
        # evaluate the loss on train/val sets and write checkpoints
        if iter_num % args.eval_interval == 0 and master_process:
            losses = Timeaware_estimate_loss(
                                        model, 
                                        ctx, 
                                        get_batch, 
                                        args.eval_iters, 
            ...
            if args.timeaware:
                print(
                f"step {iter_num}: "
                f"train loss {losses['loss/train']:.4f}, "
                f"val loss {losses['loss/val']:.4f}, "
                f"train uncertainty {losses['uncertain/train']:.4f}, "
                f"val uncertainty {losses['uncertain/val']:.4f}, "
                f"train time accuracy {losses['time_acc/train']:.4f}, "
                f"val time accuracy {losses['time_acc/val']:.4f}, "
                )
            else:
                print(
                f"step {iter_num}: "
            ...
            if losses['loss/val'] < best_val_loss:
                ...
                print(f"Saved the best model (best val loss): {best_val_loss} => {losses['loss/val']}")
                best_val_loss = losses['loss/val']
            ...
            if metric_score > best_metric_score:
                ...
                print(f"Saved the best toi model (best metric score): {best_metric_score} => {metric_score}")
        ...
        for micro_step in range(args.gradient_accumulation_steps):
            ...
            x, time_indices_x, patient_indices, y, time_indices_y = get_batch("train")
        ...
        if iter_num % args.log_interval == 0 and master_process:
            ...
            if local_iter_num >= 5:  # let the training loop settle a bit
                mfu = raw_model.estimate_mfu(args.gradient_accumulation_steps, dt)
