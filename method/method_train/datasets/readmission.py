...
from ..mimiciv.special import HadmIdMixin


class ReadmissionDataset(HadmIdMixin, InferenceDataset):
    def __init__(self, data, encode, sequence_length, is_mimic: bool = False):
        self.is_mimic = is_mimic
        patient_ids = data["patient_ids"] if is_mimic else None
        super().__init__(patient_ids=patient_ids, data=data, encode=encode, sequence_length=sequence_length)
    ...
    def __getitem__(self, idx) -> tuple[th.Tensor, dict]:
        ...
        if discharge_idx + 1 - data_start_idx + self.context_len > self.timeline_len:
            data_start_idx = discharge_idx + 1 - self.timeline_len + self.context_len
        ...
        timeline_times = self.times[data_start_idx : discharge_idx + 1]
        ...
        patient_indices = th.zeros(len(x))
        y.update(
            {
        ...
        return x, time_indices, patient_indices, y
