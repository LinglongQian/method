...
class ToiOversampleDataset(TimeAwareDataset):
    toi_indices: np.ndarray[np.int64]

    STOKENS_OF_INTEREST = [SpecialToken.DEATH, DISCHARGE_STOKEN]

    def __init__(self, data, encode, sequence_length=2048, p: float = 1):
        ...
        for idx in toi_indices:
            self.weights[idx - sequence_length : idx - half_sequence_length] = p
...
class ToiShiftingDataset(TimeAwareDataset):
    toi_indices: np.ndarray[np.int64]

    STOKENS_OF_INTEREST = [SpecialToken.DEATH, DISCHARGE_STOKEN]

    def __init__(self, data, encode, sequence_length=2048):
        super().__init__(data, encode, sequence_length)
        tokens_of_interest = th.tensor(self.encode(self.STOKENS_OF_INTEREST).astype(int))
